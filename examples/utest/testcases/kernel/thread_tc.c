...
ALIGN(RT_ALIGN_SIZE)
static char static_thread_stack[1024];
...
static void test_thread_time_slice(void)
{
    ...
    while (times --)
    {
        ...
        {
            is_success = (time_slice2 - time_slice1 < time_slice2 / 5) ? RT_TRUE : RT_FALSE;
        }
        else
        {
            is_success = (time_slice1 - time_slice2 < time_slice1 / 5) ? RT_TRUE : RT_FALSE;
        }
...
static void test_thread_priority(void)
{
    ...
    rt_kprintf("start priority test\n");
...
UTEST_TC_EXPORT(testcase, "testcases.kernel.thread_tc", utest_tc_init, utest_tc_cleanup, 60); 
